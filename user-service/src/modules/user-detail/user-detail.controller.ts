...
import {
  IUserDetailById,
  IUserDetailUpdate,
} from './interfaces/user-detail.contr.interface';
...
export class UserDetailController {
  ...
  async findAll() {
    ...
    const repeated: { userDetailts: UserDetailEntity[] } = {
      userDetailts: allAvailableUserDetailts,
    };
    const resData = new ResData<{ userDetailts: Array<UserDetailEntity> }>(
      'ok',
      200,
      repeated,
    );
  ...
  async create(data: CreateUserDetailDto) {
    ...
    return new ResData('user detail was created successfully', 201, created);
  }
  ...
  async update(data: IUserDetailUpdate) {
    ...
    return new ResData('user detail was updated successfully', 200, updated);
  }
