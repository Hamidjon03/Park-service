...
import {
  IUpdateUserTariffDto,
  IUserTariffById,
} from './interface/user-tariff.contr.interface';
...
export class UserTariffController {
  ...
  async findOne(data: IUserTariffById) {
    ...
    return new ResData('park was founded successfully', 200, userDetailById);
  }

  @GrpcMethod(USER_TARIFF_SERVICE, 'FindAll')
  async findAll() {
    const allAvailableUserTariffs = await this.userTariffService.findAll();
    const repeated: { userTariffs: UserTariffEntity[] } = {
      userTariffs: allAvailableUserTariffs,
    };
    const resData = new ResData<{ userTariffs: Array<UserTariffEntity> }>(
      'ok',
      200,
      repeated,
    );
  ...
  async create(data: CreateUserTariffDto) {
    ...
    return new ResData('user tariff was created successfully', 201, created);
  }
  ...
  async update(data: IUpdateUserTariffDto) {
    ...
    return new ResData('user tariff was updated successfully', 200, updated);
  }
  ...
  async delete(data: IUserTariffById) {
    ...
    return new ResData('user tariff was deleted successfully', 200, deleted);
  }
}
